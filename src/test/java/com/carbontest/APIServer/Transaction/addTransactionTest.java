...
public class addTransactionTest extends BaseTest {
    ...
    public void setHeaderAnduserId(){
        ...
        query.setProjectName("新增项目001");
        List<TProject> tProject=tProjectService.queryAll(query);
        this.projectid=tProject.get(0).getId();

    }

    @Test(dataProvider = "addparam",description = "交易新增测试")
    public Integer addtransactiontest(addTransactionBean addTransactionBean, String result){
        ...
        try{
            ...
            log.info("准备开始---------");
            JSONObject response=this.addTransactionService.addtransaction(addTransactionBean,result,this.headers);
        ...
        int type=tTransaction.getDataType();
        log.info("获取状态："+status);

        Assert.assertEquals(status,0,"状态校验正常");

        Assert.assertEquals(type,0,"类型校验正常");
    ...
    public Object[][] addparam()  {
        ...
        return getData("addtransactionnormal");

    }

    public  Object[][] getData(String casename){
        JSONObject jsonresult=new YamlDataHelper().getYamlDataByCase("Transaction","addTransactionData",casename);
        ...
        addTransactionBean addTransactionBean=param.toJavaObject(addTransactionBean.class);

        addTransactionBean.setProjectId(this.projectid);
