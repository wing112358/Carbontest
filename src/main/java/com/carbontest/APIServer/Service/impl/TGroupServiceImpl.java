package com.carbontest.APIServer.Service.impl;

import com.carbontest.APIServer.Service.TGroupService;
import com.carbontest.APIServer.dao.carbon.TGroupDao;
import com.carbontest.APIServer.entity.TGroup;
import org.springframework.stereotype.Service;

import javax.annotation.Resource;
import java.util.List;

/**
 * 服务商企业关系记录表(TGroup)表服务实现类
 *
 * @author makejava
 * @since 2021-07-15 10:11:37
 */
@Service("tGroupService")
public class TGroupServiceImpl implements TGroupService {
    @Resource
    private TGroupDao tGroupDao;

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    @Override
    public TGroup queryById(Integer id) {
        return this.tGroupDao.queryById(id);
    }

    @Override
    public TGroup queryAll(TGroup tGroup) {
        return this.tGroupDao.queryAll(tGroup);
    }

    /**
     * 查询多条数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    @Override
    public List<TGroup> queryAllByLimit(int offset, int limit) {
        return this.tGroupDao.queryAllByLimit(offset, limit);
    }

    /**
     * 新增数据
     *
     * @param tGroup 实例对象
     * @return 实例对象
     */
    @Override
    public TGroup insert(TGroup tGroup) {
        this.tGroupDao.insert(tGroup);
        return tGroup;
    }

    /**
     * 修改数据
     *
     * @param tGroup 实例对象
     * @return 实例对象
     */
    @Override
    public TGroup update(TGroup tGroup) {
        this.tGroupDao.update(tGroup);
        return this.queryById(tGroup.getId());
    }



    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 是否成功
     */
    @Override
    public boolean deleteById(Long id) {
        return this.tGroupDao.deleteById(id) > 0;
    }

    @Override
    public boolean deleteByUserId(Long userId) {
        return this.tGroupDao.deleteByUserId(userId) > 0;
    }
}