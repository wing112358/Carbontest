package com.carbontest.APIServer.dao.carbon;

import com.carbontest.APIServer.entity.TTransaction;
import org.apache.ibatis.annotations.Param;
import java.util.List;

/**
 * 交易记录表(TTransaction)表数据库访问层
 *
 * @author makejava
 * @since 2021-09-03 14:37:47
 */
public interface TTransactionDao {

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    TTransaction queryById(Integer id);

    /**
     * 查询指定行数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    List<TTransaction> queryAllByLimit(@Param("offset") int offset, @Param("limit") int limit);


    /**
     * 通过实体作为筛选条件查询
     *
     * @param tTransaction 实例对象
     * @return 对象列表
     */
    List<TTransaction> queryAll(TTransaction tTransaction);

    /**
     * 新增数据
     *
     * @param tTransaction 实例对象
     * @return 影响行数
     */
    int insert(TTransaction tTransaction);

    /**
     * 修改数据
     *
     * @param tTransaction 实例对象
     * @return 影响行数
     */
    int update(TTransaction tTransaction);

    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 影响行数
     */
    int deleteById(Integer id);

}