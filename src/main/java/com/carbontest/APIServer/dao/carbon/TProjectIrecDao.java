package com.carbontest.APIServer.dao.carbon;

import com.carbontest.APIServer.entity.TProjectIrec;
import org.apache.ibatis.annotations.Param;
import java.util.List;

/**
 * (TProjectIrec)表数据库访问层
 *
 * @author makejava
 * @since 2021-08-26 16:11:37
 */
public interface TProjectIrecDao {

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    TProjectIrec queryById(Long id);

    /**
     * 查询指定行数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    List<TProjectIrec> queryAllByLimit(@Param("offset") int offset, @Param("limit") int limit);


    /**
     * 通过实体作为筛选条件查询
     *
     * @param tProjectIrec 实例对象
     * @return 对象列表
     */
    TProjectIrec queryAll(TProjectIrec tProjectIrec);

    /**
     * 新增数据
     *
     * @param tProjectIrec 实例对象
     * @return 影响行数
     */
    int insert(TProjectIrec tProjectIrec);

    /**
     * 修改数据
     *
     * @param tProjectIrec 实例对象
     * @return 影响行数
     */
    int update(TProjectIrec tProjectIrec);

    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 影响行数
     */
    int deleteById(Long id);

}